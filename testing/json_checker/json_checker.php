...
$url = 'http://localhost/spm/json/';

$testcases = isset($_GET['t']) ? '-' . $_GET['t'] : '';

// The directories that contains the test cases, expected JSON output, and where to save the actual JSON output recieved
$directories = [
    'in' => 'testcases' . $testcases . '/in/',    // test cases
    'out' => 'testcases' . $testcases . '/out/',  // expected JSON output
    'yours' => 'testcases' . $testcases . '/yours/',  // where to save the actual JSON output recieved
];
...
function displayTestResult($testResult) {
        ...
        $folder = "";
        if (isset($_GET['t'])) $folder = "-" . $_GET['t'];

        echo "
            Test {$testResult['num']} '{$testResult['json api']}' $result (<a href='testcases$folder/in/{$testResult['num']}-{$testResult['json api']}.$extension' target='_blank'>in</a> | <a href='testcases$folder/out/{$testResult['num']}-{$testResult['json api']}.$extension' target='_blank'>out</a> | <a href='testcases$folder/yours/{$testResult['num']}-{$testResult['json api']}.$extension' target='_blank'>yours</a>)
        ";
